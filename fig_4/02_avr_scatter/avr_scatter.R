...
p = ggplot()
p = p + geom_point(
    data = dplyr::filter(concats, character %in% c("Control", "Both")),
    aes(
        x = avg_enrichment_d2,
        y = avg_enrichment_d5,
        color = character,
...
p = p + scale_color_manual(
    name = "",
    breaks = c("Both", "Control"),
    labels = c("A-R",
               "Control"),
    values = c("#0fa616", '#777777'))
p = p + coord_fixed(xlim = c(-5.5, 5.5), ylim = c(-5.5, 5.5))
...
p = p + theme_bw() 
p = p + theme(legend.position = c(0.8, 0.125), legend.title = element_blank(), 
