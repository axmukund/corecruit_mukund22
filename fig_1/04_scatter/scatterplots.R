...
df %>%
    dplyr::left_join(b1df,
                     by = c("domain1" = "d1_domain")) %>%
    dplyr::left_join(b2df,
                     by = c("domain2" = "d2_domain")) -> df

abbrvchar = function(d1, d2) {
    return(paste(substr(d1, 1, 1), substr(d2, 1, 1), sep = "-"))
}

df %>% # add compositions
    dplyr::mutate(composition = abbrvchar(d1_baseline_type, d2_baseline_type),)  %>%
    dplyr::mutate(
        character = case_when(
        ...
        ptype = case_when(
            str_detect(composition, "C-C") ~ "Control Pair",
            TRUE ~ "Effector Pair"
        )
    ) -> df
df$character = factor(df$character,
                      levels = c("Control", "Activator",
                                 "Repressor", "Both", "Other"))
df$ptype = factor(df$ptype, levels = c("Control Pair", "Effector Pair"))
...
df %>%
    ...
    dplyr::mutate(
        threshold = if_else(
            day == 2,
            mean_enrichment + 2 * sd_enrichment,
            mean_enrichment - 2 * sd_enrichment
        )
    ) -> control_df

df %>%
    dplyr::mutate(act_pair_hit = avg_enrichment_d2 >= -0.0236,
                  rep_pair_hit = avg_enrichment_d5 <= 0.771) -> df

# Day 2 Scatterplot
corall = cor(df$enrichment_ratio_r1_d2, df$enrichment_ratio_r2_d2, use = "complete.obs")
p2 = ggplot(data = df)
p2 = p2 + geom_point(
    data = dplyr::filter(df, ptype=="Effector Pair"),
    aes(x = enrichment_ratio_r1_d2, y = enrichment_ratio_r2_d2, color = ptype),
                     size = 0.15, alpha = 0.3)
p2 = p2 + geom_point(
    data = dplyr::filter(df, ptype!="Effector Pair"),
    aes(x = enrichment_ratio_r1_d2, y = enrichment_ratio_r2_d2, color = ptype),
...
p2 = p2 + geom_segment(
    x = 3.5,
    y = dplyr::filter(control_df, day == 2)$threshold - 3.5,
    xend = dplyr::filter(control_df, day == 2)$threshold - 3.5,
    yend = 3.5,
    linetype = "dashed",
    size = 0.5
)
p2 = p2 + annotate(
    ...
    size = 9 / .pt,
    family = "Helvetica",
    label = bquote("Pearson" ~ rho ~ "=" ~ .(round(corall, digits = 2)))
)# ~ "for all domains"))
p2 = p2 + coord_fixed(ratio = 1,
                      xlim = c(-7.5, 6.5),
                      ylim = c(-7.5, 6.5))
p2 = p2 + theme_linedraw() + theme(panel.grid = element_blank())
p2 = p2 + guides(color = guide_legend(override.aes = list(size = 3, alpha = 1.0)))
p2 = p2 + labs(
    x = bquote("Activation" ~ log[2] ~ "(ON:OFF)" ~ "R1"),
    y = bquote("Activation" ~ log[2] ~ "(ON:OFF)" ~ "R2")
)
p2 = p2 + scale_color_manual(
    name = "",
    breaks = c("Control Pair", "Effector Pair"),
    values = c("#bbbbbb", '#386cb0')
)
...
p5 = p5 + geom_point(
    data = dplyr::filter(df, ptype=="Effector Pair"),
    aes(x = enrichment_ratio_r1_d5, y = enrichment_ratio_r2_d5, color = ptype),
                     size = 0.15, alpha = 0.3)
p5 = p5 + geom_point(
    data = dplyr::filter(df, ptype!="Effector Pair"),
    aes(x = enrichment_ratio_r1_d5, y = enrichment_ratio_r2_d5, color = ptype),
...
p5 = p5 + geom_segment(
    x = 4,
    y = dplyr::filter(control_df, day == 5)$threshold - 4,
    xend = dplyr::filter(control_df, day == 5)$threshold - 4,
    yend = 4,
    linetype = "dashed",
    size = 0.5
)
...
p5 = p5 + labs(
    x = bquote("Repression" ~ log[2] ~ "(ON:OFF)" ~ "R1"),
